...
%				\label{eq:NTIC}
			...
			\subsection{Informational Closure Does not Imply Causality}\label{sec:causality}
                A surprising (but well known) result from the definition of information flow $J_t(E\rightarrow Y)$ (Eq. \ref{eq:InformationFlow}) is that information flow does not indicate causal dependency from $E_t$ to $Y_{t+1}$ and from $Y_t$ to $Y_{t+1}$. Here we consider two scenarios, called "modelling" and "passive adaptation", which were already noted by \citet{BERTSCHINGER.2006}. In both scenarios, a process can form positive NTIC ($NTIC(E\rightarrow Y)> 0$) and informational closure ($J(E\rightarrow Y)=0$), however, via different causal dependencies.
                ...
                More importantly, we argue in Appendix \ref{sec:appendix} that whenever the environment has itself more predictable dynamics than the observations, there is potential for a process to achieve higher NTIC by modelling the environment than by copying the observations. 
...
% ============================================================================ %
    ...
    % ====================================%
        ...
        % ====================================
        % Refinement construction approach
        % ====================================
        % \begin{hypothesis}
	...
	\section{Conscious Versus Unconscious Processing}\label{sec:Conscious versus Unconscious Processing}
        ...
        \subsection{Unconscious Processing}
            ...
            \subsubsection*{Processes that are not Informationally Closed}\label{sec:reflexive}
                ...
                % When the sensor processes are not deterministic, if processes dynamics is passively driven by the environmental inputs the process is not informationally closed and, thus, unconscious. 
        		...
        		% action outputs of blindsight can be directly guided by sensory inputs through 
        		stimulus-response mapping. The corresponding neural circuits are driven passively and therefore are not informationally closed. According to \ac{OurTheory} we therefore have no conscious experience of the visual stimuli.
                ...
                Note that, not all processes that are driven by the environment (passive adaptation) are unconscious. When the sensor processes are deterministic, a system can still have positive NTIC and achieve informational closure via passive adaptation. \linelabel{line:feedforward}Therefore, some passive system (for example pure feedforward networks) can potentially produce consciousness.\footnote{Since a $n$-layer feedforward network is a system with $n$-step memory it is technically appropriate to use the $n$-step memory definition of NTIC, i.e. $NTIC^m_t(E\rightarrow Y):=I(Y_{t+1}:E_t,...,E_{t-n+1})-I(Y_{t+1}:E_t,...,E_{t-n+1}|Y_t)$ \citep{BERTSCHINGER.2006}, for such systems. In this case the notion of non-deterministic input processes should be generalised to input processes with $H(\hat{e}_t|\hat_{e}_{t-1},...,\hat{e}_{t-n})>0$.}
        ...
        % 		than the because NTIC requires a form of memory $I(Y_{t+1};Y_{t})$. Without memory, a network's current state is entirely driven by the input the network receives from the environment without any influence from its own past states. Therefore, such a network is incapable of forming an NTIC. In contrast, 
            ...
            \subsubsection*{Processes that are Trivially Closed}
                ...
                This implies that isolated processes with trivial information do not contribute consciousness and should be considered being outside the information boundary of the conscious processing. 
