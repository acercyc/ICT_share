...
\begin{document}
    ...
    \section{Reply to Reviewer 1}
        ...
        \begin{ans}        	
            ...
            % For this We use the underlying $X$ with respect to $Y$ in order to illustrate the scale problem of consciousness. 
    	...
    	\begin{ans}
            ...
            % This is a difference between IIT and ICT. \\
            ...
            It is true that NTIC can be computed for every subset of a system and can also have a positive value of NTIC for multiple of those. All such subsets are thus conscious. However, while conscious, they are not necessarily the entire or whole consciousness. They may be part of a more extended consciousness. In order to find whole or entire consciousnesses we can use the algorithm in \citet{krakauer2014information, krakauer2020information}. This algorithm adds variables to or removes variables from the system if this results in a reduction of transfer entropy from the corresponding environment. This means that whole consciousnesses are maximally closed subsets of variables. We currently do not know if such maxima also are maxima of NTIC. We also currently do not know whether multiple such whole consciousnesses can overlap or not and will accept either case within ICT. In this way either exclusion emerges from our definitions or not. 
            
            
            % if a subset is not informationally closed the subset is considered as a part of the system.
...
%			A partition of $Y$ is a set $P_y$ which divides $Y$ into two subsets $P_y=\{Y_1, Y_2\}$. }    		    		    		
        ...
        \begin{ans}
        	The reviewer is right. Feed-forward networks can be conscious under ICT. Similarly, a video camera can achieve positive levels of NTIC and, therefore, be conscious. We have worked extensively on Section 5 to make this clear (see from Line:~\lineref{line:notIC}).
...
%			In our hypothesis, $Y$ is IC with respect to the microstate of the universe, X. This is critical in the hypothesis because $Y$ is IC with respect to X, implying that $Y$ is also IC with respect to its intermediate level S and its environment $E$ in Fig. \ref{fig:fullgraph}.\todo{need any change in the main text?}
    	...
    	\begin{ans}
    % 	It is true that at the finest scale, when we consider the whole universe as a system, the universe should be an informationally closed and deterministic process. However, there exists no environment to the universe and, therefore, NTIC of the universe is zero (trivially closed). 
